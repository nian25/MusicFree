    ...
    const options: IOption[] = [
        {
            ...
            async onPress() {
                try {
                    const result = await pluginManager.updatePluginWithResult(plugin);
                    // 检查更新结果
                    if (result.success) {
                        Toast.success(t("toast.updatePluginSuccess"));
                    } else {
                        // 即使success为false，也可能是已经是最新版本
                        if (result.message && (result.message.includes("已是最新版本") || result.message.includes("Already the latest version"))) {
                            Toast.success(t("checkUpdate.error.latestVersion"));
                        } else {
                            Toast.warn(result.message ?? t("toast.failToUpdatePlugin"));
                        }
                    }
                } catch (e: any) {
                    // 检查是否是已经是最新版本的错误
