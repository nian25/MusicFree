...
class Downloader extends EventEmitter<IEvents> implements IInjectable {
    ...
    private async downloadNextPendingTask() {
        ...
        try {
            if (plugin) {
                ...
                // 如果插件返回了 artwork 字段，则保存到音乐项中
                if (data?.artwork) {
                    musicItem.artwork = data.artwork;
                }
            }
            if (!url) {
    ...
    /**
     * 重新尝试下载（用于在开启移动网络下载后重新触发下载）
     */
    retryDownload(musicItems: IMusic.IMusicItem | IMusic.IMusicItem[], quality?: IMusic.IQualityKey) {
        // 直接调用下载方法
