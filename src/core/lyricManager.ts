...
class LyricManager implements IInjectable {
     ...
     * 处理歌词跳转，避免跳转时的歌词跳动问题
     */
    async handleLyricSeek(time: number) {
        // 设置跳转标志位
        this.isSeeking = true;
        
        try {
            // 执行跳转
            await this.trackPlayer.seekTo(time);
            await this.trackPlayer.play();
            
            // 更新当前歌词项到跳转位置
            if (this.lyricParser) {
                const newLyricItem = this.lyricParser.getPosition(time);
                getDefaultStore().set(currentLyricItemAtom, newLyricItem ?? null);
                ...
                if (this.appConfig.getConfig("lyric.showStatusBarLyric")) {
                    ...
                    LyricUtil.setStatusBarLyricText(
                        (newLyricItem?.lrc ?? "") +
                        (showTranslation && newLyricItem?.translation
                            ? `\n${newLyricItem?.translation ?? ""}`
                            : ""),
                    );
                }
