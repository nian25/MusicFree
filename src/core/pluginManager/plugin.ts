...
class PluginMethodsWrapper implements IPlugin.IPluginInstanceMethods {
    ...
    async getMediaSource(
        ...
        try {
            const sourceResult = await parserPlugin.instance.getMediaSource(
                musicItem,
                quality,
            );
            const { url, headers } = sourceResult ?? { url: musicItem?.qualities?.[quality]?.url };
            if (!url) {
            ...
            const result = {
                ...
                // 如果插件返回了 artwork 字段，则保留它
                ...(sourceResult?.artwork ? { artwork: sourceResult.artwork } : {}),
...
const localFilePluginDefine: IPlugin.IPluginDefine = {
    ...
    async getMediaSource(musicItem, quality) {
        if (quality === "standard") {
            ...
            if (localPath) {
                try {
                    artwork = await Mp3Util.getMediaCoverImg(localPath);
                } catch (e) {
                    console.log("获取本地文件封面失败", e);
                }
            }
