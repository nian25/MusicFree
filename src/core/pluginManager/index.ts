...
class PluginManager implements IPluginManager, IInjectable {
    ...
    private setPlugins(plugins: Plugin[]) {
        ...
        // 发出插件列表更新事件，确保UI能正确更新
        ee.emit("plugins-updated");
    }
    ...
    async installPluginFromUrl(
        ...
        try {
            ...
            if (e?.isAxiosError) {
                ...
                isNetworkError = networkErrorKeywords.some(keyword => 
     ...
     * @returns 更新结果
     */
    async updatePluginWithResult(plugin: Plugin): Promise<IInstallPluginResult> {
        const updateUrl = plugin.instance.srcUrl;
        if (!updateUrl) {
            throw new Error("没有更新源");
        }
        try {
            ...
            // 如果是网络错误，重新抛出错误以便上层处理
            if (e?.message && (e.message.includes("网络连接失败") || e.message.includes("Network Error"))) {
                throw e;
            }
            // 如果是"插件已安装"错误，转换为特定错误消息
            if (e.message === "插件已安装") {
                throw new Error(i18n.t("checkUpdate.error.latestVersion"));
            } else {
                throw e;
...
export function useSortedPlugins() {
    ...
    useEffect(() => {
        ...
        ee.on("plugins-updated", callback);
        callback();
        return () => {
            ee.off("order-updated", callback);
            ee.off("plugins-updated", callback);
        };
    }, [plugins]);
