        ...
        if (track && this.isInPlayList(track)) {
            ...
            this.pluginManagerService.getByMedia(track)
                ...
                .then(async newSource => {
                    ...
                    // 处理 artwork 字段
                    // 只有当 track 原本没有 artwork 或者 artwork 为空时，才使用 getMediaSource 的 artwork
                    if (newSource?.artwork && (!track.artwork || track.artwork.trim() === "")) {
                        track.artwork = newSource.artwork;
                    }

                    // 调用 getMusicInfo 获取补充信息
                    try {
                        const plugin = this.pluginManagerService.getByMedia(track);
                        const info = await plugin?.methods?.getMusicInfo?.(track);
                        if (info) {
                            // 只有当 track 原本没有 artwork 时，才使用 getMusicInfo 的 artwork
                            if (info.artwork && (!track.artwork || track.artwork.trim() === "")) {
    ...
    async play(
        ...
        try {
            ...
            try {
                ...
                // 如果获取到信息且有 artwork，添加到 musicItem，其他信息在后面合并，这里封面提前可以增加体验
                if (info?.artwork) {
                    musicItem.artwork = info.artwork;
                }
            } catch (error) {
            ...
            // // 10.1 获取媒体源中的补充信息（如 artwork）
            // // 注意：只有当 getMusicInfo 没有返回 artwork 时，才使用 getMediaSource 的 artwork
            // // 原是用来增加getMediaSource函数对artwork的支持，现弃用
            // if (source?.artwork && !info?.artwork) {
            //     info = {
            //         ...(info ?? {}),
            ...
            if (info && this.isCurrentMusic(musicItem)) {
                ...
                // 持久化更新后的音乐项
                PersistStatus.set("music.musicItem", mergedTrack as IMusic.IMusicItem);
            }
        } catch (e: any) {
            ...
            if (
                ...
                if (getCurrentDialog()?.name !== "SimpleDialog") {
                    showDialog("SimpleDialog", {
                        title: "流量提醒",
                        content:
                            "当前非WIFI环境，是否开启移动网络播放？",
                        okText: "开启",
                        cancelText: "取消",
                        onOk: () => {
                            // 开启移动网络播放功能
                            this.configService.setConfig("basic.useCelluarNetworkPlay", true);
                            // 重新尝试播放
                            this.play(musicItem, forcePlay);
                        }
                    });
    ...
    async changeQuality(newQuality: IMusic.IQualityKey): Promise<boolean> {
        ...
        try {
            ...
            if (this.isCurrentMusic(musicItem)) {
                ...
                // 处理 artwork 字段
                let trackSource = newSource;
                if (newSource?.artwork) {
                    trackSource = {
                        ...newSource,
                        artwork: newSource.artwork
                    };
                }
                
                await this.setTrackSource(
                    this.mergeTrackSource(musicItem, trackSource) as unknown as Track,
